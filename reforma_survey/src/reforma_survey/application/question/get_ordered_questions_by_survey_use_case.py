...
class GetOrderedQuestionsBySurveyUseCase:
    ...
    async def execute(self, survey_id: UUID) -> List[Question]:
        log_info(f"Начало получения упорядоченных вопросов опроса {survey_id}", service="survey-service")

        async with SessionLocal() as db:
            async with db.begin():
                try:
                    questions = await self.repository.get_by_survey_ordered(survey_id)
                    log_info(f"Получено {len(questions)} упорядоченных вопросов для опроса {survey_id}", service="survey-service")
                    return questions
                except Exception as e:
                    log_error(f"Ошибка получения упорядоченных вопросов опроса {survey_id}: {e}", service="survey-service")
                    raise