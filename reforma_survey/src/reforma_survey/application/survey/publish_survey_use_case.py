...
class PublishSurveyUseCase:
    ...
    async def execute(self, survey_id: UUID) -> Survey:
        log_info(f"Начало публикации опроса {survey_id}", service="survey-service")

        async with SessionLocal() as db:
            async with db.begin():
                try:
                    updated = await self.repository.publish(survey_id)
                    log_info(f"Опрос {survey_id} успешно опубликован", service="survey-service")
                    return updated
                except ValueError as ve:
                    log_warning(f"Не удалось опубликовать опрос {survey_id}: {ve}", service="survey-service")
                    raise
