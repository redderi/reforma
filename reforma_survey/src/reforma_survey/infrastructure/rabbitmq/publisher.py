...
class EventPublisher:
    ...
    async def publish_event(
        ...
        try:
            ...
            log_info(f"[Publisher] Sending event to {exchange_name} rk={routing}", service="survey-service")
            await self.rabbit.publish(exchange_name, routing, message)
            log_info("[Publisher] Event sent successfully", service="survey-service")
