...
class S3StorageClient(BaseStorageClient):
    ...
    async def upload_fileobj(self, file_obj: BinaryIO, object_name: str) -> str:
        ...
        try:
            self.client.upload_fileobj(
                Fileobj=file_obj, Bucket=self.bucket, Key=object_name
            )
            log_info(f"Файл успешно загружен: {object_name}", service="report-service")
            return object_name
        except ClientError as e:
            log_error(
                f"Ошибка загрузки файла {object_name}: {e}", service="report-service"
            )
    ...
    async def generate_presigned_url(
        ...
        try:
            ...
            log_error(
                f"Ошибка генерации presigned-ссылки для {object_name}: {e}",
                service="report-service",
            )
    ...
    async def delete_object(self, object_name: str) -> None:
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "404":
                log_warning(
                    f"Объект {object_name} уже не существует", service="report-service"
                )
            else:
                log_error(
                    f"Ошибка удаления объекта {object_name}: {e}",
                    service="report-service",
                )
    ...
    async def object_exists(self, object_name: str) -> bool:
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "404":
                return False
            log_error(
                f"Ошибка проверки объекта {object_name}: {e}", service="report-service"
            )
            raise
